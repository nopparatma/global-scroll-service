...
class RedisService {
    ...
    async incrementCountryHeight(countryCode: string, amount: number): Promise<number> {
    ...
    async getCountryHeights(countries: string[]): Promise<Record<string, string>> {
    ...
    async getAllCountryHeights(): Promise<Record<string, string>> {
        ...
        const countryKeys = keys.filter(k => k !== 'global:height' && k.startsWith('global:height:'));
        ...
        countryKeys.forEach((key, i) => {
            ...
            if (parseInt(height) > 0) { // Only include countries with height > 0
