...
export const initializeSocketIO = (io: Server) => {
    io.on('connection', async (socket: Socket) => {
        try {
            ...
            logger.info(`Country determined: ${countryCode} (manual: ${!!manualCountryCode})`);

            // 3. Find/Create User
            const user = await userService.findOrCreateByDeviceId(deviceId, countryCode);
            ...
            socket.on('scroll_batch', async (data) => {
                try {
                    ...
                    const result = await gameService.processScrollBatch(user.id, countryCode, delta, timeDiff);

                    if (result) {
                        // Broadcast update (throttled in real app)
                        // For now, we rely on the 'tick' loop to broadcast
                    }

                    lastBatchTime = now;
                } catch (err) {
                    logger.warn(`Invalid scroll data from ${user.id}`);
                }
            });
